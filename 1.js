...
const VeriYonetim = require('../veri_yonetim');
...
module.exports = {
  ...
  handleQueryModal:  async function(interaction, context) {
    ...
    // Interaction durumu kontrolü - zaten yanıt verildiyse çık
    ...
    try {
      ...
      try {
        ...
        // Interaction durumunu kontrol et ve uygun şekilde yanıt ver
      ...
      try {
        // Interaction durumuna göre uygun yanıt ver
        if (! interaction.replied && ! interaction.deferred) {
          await interaction.reply({ embeds: [errorEmbed], ephemeral:  true });
        } else {
          await interaction. editReply({ embeds: [errorEmbed], components: [] });
